...
describe('apiClient', () => {
  it('apiGet returns parsed json on success', async () => {
    const fake = { ok: true, json: async () => ({ hello: 'world' }) } as any;
    globalThis.fetch = vi.fn(() => Promise.resolve(fake)) as any;
    const res = await apiGet('/test');
    expect(res).toEqual({ hello: 'world' });
  });

  it('apiPost sends body and returns parsed json', async () => {
    const fake = { ok: true, json: async () => ({ ok: true }) } as any;
    const fetchMock = vi.fn(() => Promise.resolve(fake)) as any;
