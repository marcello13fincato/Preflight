          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              {records.map((record, idx) => (
                <details key={record.id} className={"fade-in" + (idx > 0 ? " fade-in-delay" : "")}
                  style={{
                    background: "var(--color-bg)",
                    border: "1px solid var(--color-border)",
                    borderRadius: "12px",
                  ...
                  <summary style={{
                    ...
                    <span style={{
                      ...
                      fontWeight: 700,
                      flexShrink: 0,
                    }}>{record.type === "prospect" ? "👤" : record.type === "dm" ? "💬" : "📋"}</span>
                    <span style={{ flex: 1 }}>{record.type} · {new Date(record.createdAt).toLocaleString("it-IT", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</span>
                  </summary>
                  <pre style={{
