...
export default function MarketingCTA({ className = "" }: { className?: string }) {
  return (
    <div className={`flex flex-wrap items-center justify-center gap-3 ${className}`}>
      ...
      <Link href="/login" className="btn-secondary rounded-xl px-5 py-3">Accedi</Link>
      <Link href="/dashboard" className="text-sm text-muted underline">Vai alla dashboard</Link>
    </div>
  );
}
